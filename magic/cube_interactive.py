...
class InteractiveCube(plt.Axes):
    ...
    def _solve_cube(self, *args):
        ...
        cube = rubiks.RubiksCube(self.cube.N, None)
        for move in move_list:
            assert(move[2] == 0)
            face_lookup = {'U': rubiks.FaceType.U,
                           'D': rubiks.FaceType.D,
                           'L': rubiks.FaceType.L,
                           'R': rubiks.FaceType.R,
                           'F': rubiks.FaceType.F,
                           'B': rubiks.FaceType.B}
            rottype_lookup = {1: rubiks.RotType.CLOCK,
                             -1: rubiks.RotType.COUNTER,
                              2: rubiks.RotType.FLIP}
            face = face_lookup[move[0]]
            rottype = rottype_lookup[round(move[1])]
            move = rubiks.get_move(face, rottype)
            
            print(move)
            cube.do_move(move)
        
        print('-----------')
        solve = solver.RubiksSolver(cube, len(move_list), optimize=True)
        moves = solve.solve()
        print(moves)

        for move in moves:
            face = next(k for k, v in face_lookup.items()
                    if v == rubiks.get_face(move))
            rottype = next(k for k, v in rottype_lookup.items()
                    if v == rubiks.get_rot_type(move))

            self.rotate_face(face, rottype, 0, steps=12)

        # for (face, n, layer) in move_list[::-1]:
            # self.rotate_face(face, -n, layer, steps=7)
